...
_StatutConfig _configStatut(String statut) {
  ...
  if (s == 'wave') {
    return const _StatutConfig('🌊', _DS.skyLight, _DS.sky, Color(0xFFBBDEFB));
  }
  if (s == 'orange money' || s == 'om') {
    return const _StatutConfig('🟠', _DS.orangeLight, _DS.orange, Color(0xFFFFCCBC));
  }
...
String _labelStatut(String statut) {
  ...
  if (s == 'à crédit' || s == 'a credit' ||
      s == 'crédit'   || s == 'credit')                        return 'À CRÉDIT';
  return 'PAYÉ EN CASH';
}
...
class _VentesPageState extends State<VentesPage>
  ...
  // Cache des items par vente_id — image_url incluse directement
  ...
  // ── CHARGEMENT ITEMS ────────────────────────────────────────────────────────
  // ✅ CORRECTION : on lit image_url directement dans ventes_items
  //    (la colonne est maintenant sauvegardée lors de la vente)
  Future<List<Map<String, dynamic>>> _chargerItems(int venteId) async {
    ...
    try {
      final rawItems = await Supabase.instance.client
          .from('ventes_items')
          .select()          // image_url est maintenant dans cette table
          .eq('vente_id', venteId)
          .order('id');

      final items = List<Map<String, dynamic>>.from(rawItems);

      // ── Fallback : si image_url manque (anciennes ventes), on la cherche
      //    dans la table products via product_id ──────────────────────────────
      final missingIds = items
          .where((i) =>
              (i['image_url'] == null || (i['image_url'] as String).isEmpty) &&
              i['product_id'] != null &&
              (i['product_id'] as String).isNotEmpty)
      ...
      if (missingIds.isNotEmpty) {
        try {
          ...
          // Injecter dans les items manquants
          for (final item in items) {
            if (item['image_url'] == null ||
                (item['image_url'] as String).isEmpty) {
              final pid = item['product_id']?.toString();
              if (pid != null && imgMap.containsKey(pid)) {
                item['image_url'] = imgMap[pid];
              }
            }
          }
          debugPrint('✅ Fallback images: ${imgMap.length}/${missingIds.length}');
        } catch (e) {
          debugPrint('⚠️ Fallback images échoué: $e');
        }
      }
    ...
    final result = _ventes.where((v) {
      ...
      if (_filtreStatut != 'tous') {
        ...
        switch (_filtreStatut) {
          case 'paye':
            if (s == 'wave' || s == 'orange money' || s == 'om' ||
                s == 'à crédit' || s == 'a credit' ||
                s == 'crédit'   || s == 'credit') return false;
          case 'wave':
  ...
  void _setFiltrePeriode(String v) {
    setState(() => _filtrePeriode = v);
    _appliquerFiltres();
  }

  void _setFiltreStatut(String v) {
    setState(() => _filtreStatut = v);
    _appliquerFiltres();
  }

  // ── STATS ──────────────────────────────────────────────────────────────────
  double get _totalFiltre => _ventesFiltrees.fold(
      0.0, (s, v) => s + ((v['total_montant'] ?? 0) as num).toDouble());
    ...
    return Scaffold(
      ...
      body: Column(children: [
        ...
        Expanded(
          child: _loading
              ? const Center(
                  child: CircularProgressIndicator(color: _DS.emeraldMid))
              : _ventesFiltrees.isEmpty
                  ...
                  : RefreshIndicator(
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (_, i) =>
                            _buildCarteVente(_ventesFiltrees[i]),
                      ),
                    ),
        ),
      ]),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      '${_ventes.length} vente${_ventes.length > 1 ? 's' : ''} au total',
                      style: TextStyle(
            ...
            child: Padding(
              ...
              child: Row(children: [
                _StatCard(
                  ...
                  iconColor: _DS.goldLight,
                  label: "Aujourd'hui",
                  value: '${_fmt(_totalJour)} F',
                  valueColor: _DS.goldLight,
                ),
                ...
                _StatCard(
                  ...
                  label: 'Crédit impayé',
                  value: '${_fmt(_totalCreditEnCours)} F',
    ...
    return Container(
      ...
      child: Column(children: [
        Padding(
          ...
          child: Row(children: [
            Expanded(child: _FiltreChipWide(
              key: const ValueKey('p1'),
              label: 'Auj.',
              icon: Icons.wb_sunny_rounded,
              iconColor: _DS.gold,
              accentColor: _DS.emeraldMid,
              selected: _filtrePeriode == 'aujourd_hui',
              onTap: () => _setFiltrePeriode('aujourd_hui'),
            )),
            ...
            Expanded(child: _FiltreChipWide(
              ...
              iconColor: _DS.sky,
              accentColor: _DS.emeraldMid,
              selected: _filtrePeriode == 'semaine',
              onTap: () => _setFiltrePeriode('semaine'),
            )),
            const SizedBox(width: 6),
            Expanded(child: _FiltreChipWide(
              ...
              accentColor: _DS.emeraldMid,
              selected: _filtrePeriode == 'mois',
              onTap: () => _setFiltrePeriode('mois'),
            )),
            const SizedBox(width: 6),
            Expanded(child: _FiltreChipWide(
              ...
              iconColor: _DS.slate,
              accentColor: _DS.emeraldMid,
              selected: _filtrePeriode == 'tout',
              onTap: () => _setFiltrePeriode('tout'),
            )),
          ]),
        ),
        Padding(
          ...
          child: Row(children: [
            Expanded(child: _FiltreChipWide(
              ...
              onTap: () => _setFiltreStatut('tous'),
            )),
            const SizedBox(width: 6),
            Expanded(child: _FiltreChipWide(
              key: const ValueKey('s2'),
              label: 'Cash',
              assetPath: _PaymentAssets.cash,
              selected: _filtreStatut == 'paye',
              onTap: () => _setFiltreStatut('paye'),
              accentColor: _DS.emeraldMid,
            )),
            const SizedBox(width: 6),
            Expanded(child: _FiltreChipWide(
              ...
              label: 'Wave',
              assetPath: _PaymentAssets.wave,
              selected: _filtreStatut == 'wave',
              onTap: () => _setFiltreStatut('wave'),
              accentColor: const Color(0xFF1A6DFF),
            )),
            const SizedBox(width: 6),
            Expanded(child: _FiltreChipWide(
              key: const ValueKey('s4'),
              label: 'OM',
              assetPath: _PaymentAssets.om,
              selected: _filtreStatut == 'om',
              onTap: () => _setFiltreStatut('om'),
            ...
            Expanded(child: _FiltreChipWide(
              ...
              label: 'Crédit',
              assetPath: _PaymentAssets.credit,
              selected: _filtreStatut == 'credit',
              onTap: () => _setFiltreStatut('credit'),
              accentColor: _DS.coral,
              badge: _nbCreditsFiltre > 0 ? '$_nbCreditsFiltre' : null,
            )),
          ]),
        ),
  ...
  Widget _buildCarteVente(Map<String, dynamic> vente) {
    ...
    final bool isCredit  = _normaliser(statut) == 'à crédit' ||
        _normaliser(statut) == 'crédit' ||
        _normaliser(statut) == 'credit';
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: isCredit
                ? _DS.coral.withValues(alpha: 0.3)
                : _DS.mist,
              ...
              child: Column(
                ...
                children: [
                  if (nom != null && nom.isNotEmpty)
                    Text(nom,
                        style: _DS.label(14, c: _DS.ink, fw: FontWeight.w800),
                    ...
                    if (nom != null && nom.isNotEmpty)
                      Container(
                        ...
                        decoration: BoxDecoration(
                            ...
                            border: Border.all(color: _DS.mist)),
                        child: Row(children: [
                    ...
                    child: Padding(
                      ...
                      child: Center(
                          child: CircularProgressIndicator(
                              color: _DS.emeraldMid, strokeWidth: 2)),
                    ),
                  )
                else
                  SliverList(
                    delegate: SliverChildBuilderDelegate(
                      (_, i) => _buildLigneDetail(items[i]),
                ...
                SliverToBoxAdapter(
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(
                          color: _DS.emeraldMid.withValues(alpha: 0.2)),
                    ),
                    child: Row(
  ...
  // ── ✅ LIGNE DÉTAIL — image_url lue directement depuis ventes_items ─────────
  Widget _buildLigneDetail(Map<String, dynamic> item) {
    final String  nom       = item['nom_produit']   as String? ?? 'Produit';
    ...
    // ✅ image_url sauvegardée directement dans ventes_items
    final String? imageUrl  = item['image_url']     as String?;

    final String fallbackEmoji = cat == 'Alimentaire' ? '🍞'
        : cat == 'Boissons' ? '🥤'
        : cat == 'Hygiène'  ? '🧴'
        : '📦';
    ...
    final String fallbackAsset =
        assetMap[cat] ?? 'lib/assets/categories/produit.png';
      ...
      child: Row(children: [
        ...
        Expanded(
          child: Column(
            ...
            children: [
              Text(nom,
                  style: _DS.label(13, c: _DS.ink, fw: FontWeight.w700)),
              const SizedBox(height: 2),
              Text('${_fmt(pu)} F × $qty',
                  style: _DS.label(11,
  ...
  // ── Widget image : network → asset local → emoji ───────────────────────────
  Widget _buildProduitImage({
    ...
    if (imageUrl != null && imageUrl.isNotEmpty) {
      return Image.network(
        ...
        errorBuilder: (_, __, ___) =>
            _buildAssetImage(fallbackAsset, fallbackEmoji, size),
      );
    }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          Container(
            ...
            decoration: const BoxDecoration(
                color: _DS.mist, shape: BoxShape.circle),
            child: const Center(
          ...
          Text('Changez les filtres ou\ncréez votre première vente',
              ...
              style:
                  _DS.label(13, c: _DS.slate, fw: FontWeight.w400)),
...
// ─────────────────────────────────────────────────────────────────────────────
// CHEMINS ASSETS PAIEMENT
// ─────────────────────────────────────────────────────────────────────────────
class _PaymentAssets {
  static const cash   = 'lib/assets/categories/cash.jpeg';
  static const wave   = 'lib/assets/categories/wave.png';
  static const om     = 'lib/assets/categories/om.jpg';
  static const credit = 'lib/assets/categories/credit.png';
  ...
  static String forStatut(String statut) {
    ...
    if (s == 'à crédit' || s == 'a credit' ||
        s == 'crédit'   || s == 'credit') return credit;
    return cash;
  }
}

// ─────────────────────────────────────────────────────────────────────────────
// WIDGET : LOGO PAIEMENT
// ─────────────────────────────────────────────────────────────────────────────
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: cfg.bg,
        borderRadius: radius,
        border: Border.all(color: cfg.borderClr, width: 1),
      ),
      child: ClipRRect(
        borderRadius: radius,
        child: Image.asset(
          _PaymentAssets.forStatut(statut),
          width: size, height: size,
          fit: BoxFit.cover,
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: cfg.bg,
        borderRadius: BorderRadius.circular(20),
        border: Border.all(color: cfg.borderClr, width: 0.5),
      ),
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(14),
          border: Border.all(
            color: selected ? accentColor : _DS.mist,
            width: selected ? 2 : 1,
          ),
          boxShadow: selected
              ? [
    ...
    child: Center(
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        Icon(icon ?? Icons.payments_rounded,
            size: 20,
            color: selected ? accentColor : (iconColor ?? _DS.slate)),
        const SizedBox(height: 4),
        Text(label,
            style: TextStyle(
                fontSize: 10,
                fontWeight: FontWeight.w700,
                color: selected ? accentColor : _DS.slate),
            textAlign: TextAlign.center,
            maxLines: 1,
            overflow: TextOverflow.ellipsis),
      ]),
