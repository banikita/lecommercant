...
class _DashboardPageState extends State<DashboardPage>
  ...
  // --- Ta palette de couleurs intacte ---
  ...
  // --- Chargement des données (SQLite + Refresh possible) ---
  Future<void> _chargerDonnees() async {
    setState(() => _loading = true);
    try {
      final transactions = await _db.chargerTransactionsRecentes(widget.commercantId, limite: 5);
        ...
        AnimatedBuilder(
            ...
            builder: (_, __) => _sidebarOpen
                ? GestureDetector(
                    onTap: _closeSidebar,
                    child: Container(
                        color: Colors.black.withOpacity(0.42 * _sidebarAnim.value)))
      ...
      child: Column(children: [
        ...
        Container(
            ...
            child: Row(children: [
              ...
              Container(
                  height: 36, width: 0.5, color: Colors.white.withOpacity(0.3)),
              Expanded(
                  child: Column(children: [
...
// ────────────────────────────────────────────────────────────
//  WIDGETS RÉUTILISABLES (BOUTONS, CARTES)
// ────────────────────────────────────────────────────────────
...
class _TxCard extends StatelessWidget {
  ...
  String get _nom => tx['nom_client'] as String? ?? 'Client Inconnu';
    ...
    return Container(
        ...
        child: Row(children: [
          Container(
              ...
              child: const Center(child: Icon(Icons.receipt_long_rounded, color: Color(0xFF6B7280), size: 18))),
          const SizedBox(width: 12),
          Expanded(
              child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                ...
                Container(
                    ...
                    decoration: BoxDecoration(color: _typeBg[_type] ?? _typeBg['vente'], borderRadius: BorderRadius.circular(20)),
                    child: Text(_typeLbl[_type] ?? 'Action',
                        style: TextStyle(
                            fontSize: 10,
                            fontWeight: FontWeight.w700,
                            color: _typeClr[_type] ?? _typeClr['vente']))),
              ])),
...
class _SItem extends StatelessWidget {
  ...
  final VoidCallback onTap;
  const _SItem({required this.emoji, required this.label, required this.color, required this.onTap, this.textColor = const Color(0xFF1A1A1A)});
