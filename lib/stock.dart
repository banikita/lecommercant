  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Scanner un produit', style: TextStyle(fontWeight: FontWeight.bold)),
        actions: [
          IconButton(
                ...
                children: [
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      border: Border.all(color: const Color(0xFF0F6E56), width: 3),
            ...
            child: Column(
              children: [
                ...
                TextButton.icon(
                  ...
                  label: const Text('Annuler', style: TextStyle(color: Colors.white, fontSize: 16)),
                ),
              ],
            ),
          ),
...
class _ScanLineState extends State<_ScanLine> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _position = Tween<double>(begin: -120, end: 120).animate(CurvedAnimation(parent: _anim, curve: Curves.easeInOut));
  }
      ...
      builder: (_, __) => Transform.translate(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [BoxShadow(color: const Color(0xFF1A6B3C).withOpacity(0.6), blurRadius: 6)],
          ),
        ),
      ),
    );
...
class _StockPageState extends State<StockPage> {
  ...
  final List<String> categories = [
    'Tous', 'Alimentaire', 'Boissons', 'Hygiène', 'Électronique', 'Vêtements', 'Autre'
  ];
  ...
  List<Product> products = [
    ...
    Product(id: '005', name: 'Téléphone Samsung A15', category: 'Électronique', quantity: 4, price: 120000, minStock: 2, addedDate: DateTime.now().subtract(const Duration(days: 3))),
    Product(id: '006', name: 'Jus de Mangue 33cl', category: 'Boissons', quantity: 1, price: 350, minStock: 6, addedDate: DateTime.now().subtract(const Duration(days: 1))),
  ];
  ...
  // ── SCANNER RÉEL ──
  void _openScanner({TextEditingController? prefill, bool searchMode = false}) {
    Navigator.push(
      context,
      MaterialPageRoute(
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  ...
                  content: Row(children: [
                    ...
                    Expanded(child: Text('Code scanné : $code')),
                  ]),
                ),
              );
            } else {
              ...
              if (found.isEmpty) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    ...
                    content: Text('Produit non trouvé pour : $code'),
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    backgroundColor: green,
                    content: Text('Produit trouvé : ${found.first.name}'),
                  ),
                );
              }
            }
  ...
  PreferredSizeWidget _buildAppBar() {
    return AppBar(
      ...
      title: _currentIndex == 0
          ...
          : const Text('Gestion de Stock', style: TextStyle(fontWeight: FontWeight.bold)),
      actions: [
        if (_currentIndex == 0)
          IconButton(
            icon: const Icon(Icons.qr_code_scanner),
            onPressed: () => _openScanner(searchMode: true),
            tooltip: 'Scanner',
          ),
        Stack(
          children: [
            ...
            if (lowStockCount > 0)
              Positioned(
                ...
                child: Container(
                  ...
                  child: Text('$lowStockCount', style: const TextStyle(color: Colors.white, fontSize: 10)),
                ),
              ),
          ],
        ),
  ...
  Widget _buildBottomNav() {
    return BottomNavigationBar(
      ...
      items: [
        ...
        const BottomNavigationBarItem(icon: Icon(Icons.bar_chart), label: 'Statistiques'),
          ...
          child: Row(
            children: [
              ...
              Text('${filteredProducts.length} produit(s)',
                  style: const TextStyle(color: green, fontSize: 13, fontWeight: FontWeight.w500)),
              const Spacer(),
              if (_searchQuery.isNotEmpty)
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    child: Row(children: [
                      Text('"$_searchQuery"', style: const TextStyle(color: green, fontSize: 12)),
        ...
        Expanded(
          child: filteredProducts.isEmpty
              ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                  ...
                  Text('Aucun produit trouvé', style: TextStyle(color: Colors.grey[600], fontSize: 16)),
                ]))
              : ListView.builder(
  ...
  Widget _buildProductCard(Product p) {
    return Card(
      ...
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(12),
        side: BorderSide(
          color: p.isLowStock ? Colors.red.withOpacity(0.4) : Colors.grey.withOpacity(0.15),
      ...
      child: ListTile(
        ...
        leading: Container(
          ...
          child: Icon(_categoryIcon(p.category), color: p.isLowStock ? Colors.red : green, size: 24),
        ),
        title: Row(
          children: [
            ...
            if (p.isLowStock)
              Container(
                ...
                child: const Text('Stock bas', style: TextStyle(color: Colors.red, fontSize: 11, fontWeight: FontWeight.bold)),
              ),
          ],
        ),
        subtitle: Column(
          ...
          children: [
            ...
            Row(children: [
              const Icon(Icons.category, size: 12, color: green),
              const SizedBox(width: 4),
              Text(p.category, style: const TextStyle(color: green, fontSize: 12)),
              const SizedBox(width: 12),
              Icon(Icons.calendar_today, size: 12, color: Colors.grey[500]),
              const SizedBox(width: 4),
              Text(_formatDate(p.addedDate), style: TextStyle(color: Colors.grey[500], fontSize: 12)),
              if (p.barcode != null) ...[
                const SizedBox(width: 12),
                const Icon(Icons.qr_code, size: 12, color: Colors.grey),
                const SizedBox(width: 4),
                Text(p.barcode!, style: const TextStyle(color: Colors.grey, fontSize: 11)),
              ],
            ]),
            const SizedBox(height: 4),
            Row(children: [
              ...
              Text(' / min ${p.minStock}', style: TextStyle(color: Colors.grey[400], fontSize: 12)),
              const Spacer(),
        ...
        trailing: PopupMenuButton<String>(
          ...
          itemBuilder: (_) => [
            const PopupMenuItem(value: 'add_stock', child: Row(children: [Icon(Icons.add_circle, color: Colors.green, size: 18), SizedBox(width: 8), Text('Ajouter stock')])),
            const PopupMenuItem(value: 'edit', child: Row(children: [Icon(Icons.edit, color: Colors.blue, size: 18), SizedBox(width: 8), Text('Modifier')])),
            const PopupMenuItem(value: 'delete', child: Row(children: [Icon(Icons.delete, color: Colors.red, size: 18), SizedBox(width: 8), Text('Supprimer')])),
          ],
        ),
      ),
    );
  ...
  // ─────────────────────────────────────────
  Widget _buildStats() {
    final topProducts = [...products]..sort((a, b) => (b.price * b.quantity).compareTo(a.price * a.quantity));
        ...
        children: [
          GridView.count(
            ...
            children: [
              ...
              _statCard('Valeur totale', '${_formatPrice(totalValue)} F', Icons.account_balance_wallet, Colors.blue),
          ...
          ...([...products]..sort((a, b) => b.addedDate.compareTo(a.addedDate)))
              .take(3).map((p) => _recentProductRow(p)),
        ],
      ),
    );
  }
  ...
  Widget _categoryBar(String category, int count, int total) {
    ...
    return Padding(
      ...
      child: Column(children: [
        Row(children: [
          Icon(_categoryIcon(category), size: 14, color: green),
          const SizedBox(width: 6),
          Text(category, style: const TextStyle(fontSize: 13)),
          const Spacer(),
          Text('$count produit(s)', style: TextStyle(color: Colors.grey[600], fontSize: 13)),
        ]),
        const SizedBox(height: 4),
        ClipRRect(
  ...
  Widget _topProductRow(Product p) {
    return Container(
      margin: const EdgeInsets.only(bottom: 8),
      padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10),
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(10)),
      child: Row(children: [
        Icon(_categoryIcon(p.category), size: 18, color: green),
  ...
  Widget _recentProductRow(Product p) {
    return Container(
      margin: const EdgeInsets.only(bottom: 8),
      padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10),
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(10)),
      child: Row(children: [
        const Icon(Icons.fiber_new, size: 18, color: green),
        const SizedBox(width: 10),
        Expanded(child: Text(p.name, style: const TextStyle(fontSize: 13))),
        Text(_formatDate(p.addedDate), style: TextStyle(color: Colors.grey[500], fontSize: 12)),
      ]),
    ...
    if (lowStockProducts.isEmpty) {
      return Center(
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          const SizedBox(height: 16),
          const Text('Aucune alerte !', style: TextStyle(fontSize: 18, color: green, fontWeight: FontWeight.bold)),
          const SizedBox(height: 8),
          Text('Tous vos stocks sont suffisants.', style: TextStyle(color: Colors.grey[600])),
        ]),
      );
    }
    return Column(children: [
      Container(
        width: double.infinity, color: Colors.red[50],
          ...
          child: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                ...
                const SizedBox(width: 10),
                const Text('Nouveau produit', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: green)),
              ]),
              const SizedBox(height: 16),
              ...
              const SizedBox(height: 10),
              Row(children: [
                Expanded(child: _inputField(qtyCtrl, 'Quantité *', Icons.numbers, isNumber: true)),
                const SizedBox(width: 10),
                Expanded(child: _inputField(minCtrl, 'Stock min', Icons.low_priority, isNumber: true)),
              ]),
              const SizedBox(height: 10),
              _inputField(priceCtrl, 'Prix (FCFA) *', Icons.attach_money, isNumber: true),
              const SizedBox(height: 10),
              Row(children: [
                Expanded(child: _inputField(barcodeCtrl, 'Code-barres (optionnel)', Icons.qr_code)),
                const SizedBox(width: 8),
                Container(
                  decoration: BoxDecoration(
                    color: greenLight,
                  ...
                  onPressed: () {
                    if (nameCtrl.text.isEmpty || qtyCtrl.text.isEmpty || priceCtrl.text.isEmpty) {
                      ScaffoldMessenger.of(ctx).showSnackBar(
                          const SnackBar(content: Text('Remplissez les champs obligatoires (*)'), backgroundColor: Colors.red));
                      return;
                    }
                    setState(() {
                    ...
                    Navigator.pop(ctx);
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(backgroundColor: green, content: Text('${nameCtrl.text} ajouté avec succès !')),
                    );
                  },
                ),
              ),
          ...
          child: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                ...
                const SizedBox(width: 10),
                const Text('Modifier produit', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: green)),
              ]),
              const SizedBox(height: 16),
              _inputField(nameCtrl, 'Nom du produit', Icons.label),
              const SizedBox(height: 10),
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  onPressed: () {
                    ...
                    Navigator.pop(ctx);
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(backgroundColor: green, content: Text('${p.name} modifié avec succès !')),
                    );
                  },
                ),
              ),
  ...
  void _showAddStockDialog(Product p) {
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        title: const Text('Ajouter stock', style: TextStyle(color: green)),
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          Text('${p.name}\nStock actuel : ${p.quantity}', style: const TextStyle(fontSize: 14)),
          const SizedBox(height: 12),
          ...
          ElevatedButton(
            ...
            onPressed: () {
              ...
              if (add > 0) {
                ...
                Navigator.pop(context);
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(backgroundColor: green, content: Text('+$add ajouté à ${p.name}')),
                );
              }
            },
  ...
  void _confirmDelete(Product p) {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
        title: const Text('Supprimer produit', style: TextStyle(color: Colors.red)),
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () {
              ...
              Navigator.pop(context);
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(backgroundColor: Colors.red, content: Text('Produit supprimé')),
              );
            },
  ...
  IconData _categoryIcon(String cat) {
    switch (cat) {
      case 'Alimentaire': return Icons.rice_bowl;
      case 'Boissons': return Icons.local_drink;
      case 'Hygiène': return Icons.soap;
      case 'Électronique': return Icons.devices;
      case 'Vêtements': return Icons.checkroom;
