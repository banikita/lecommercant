...
class AppDatabase {
  ...
  Future<Map<String, dynamic>?> chargerCommercantParId(int id) async {
  final db = await database;
...
Future<bool> estBloque(int id) async {
  final db = await database;
  ...
  if (result.isNotEmpty) {
    return result.first['bloque'] == 1;
  }
  return false;
}

Future<void> bloquerCompte(int id) async {
  final db = await database;
