...
class _LignePanier {
  ...
  final String? imageUrl;   // ← URL réelle du produit
...
class NouvelleVentePage extends StatefulWidget {
  ...
  const NouvelleVentePage({super.key, required this.commercantId});
...
class _NouvelleVentePageState extends State<NouvelleVentePage>
  ...
  Future<void> _ajouterParBarcode(String barcode) async {
    try {
      ...
      if (result == null) {
        _toast('Produit introuvable pour ce code', erreur: true);
        return;
      }
      final stock = (result['quantity'] as int?) ?? 0;
      if (stock <= 0) {
        _toast('Stock vide pour ${result['name']}', erreur: true);
        return;
      }
  ...
  Future<void> _ouvrirStock() async {
    ...
    for (final item in result) {
      _ajouterAuPanier({
        ...
        'image_url': item['image_url'], // ← on propage l'image depuis le stock
    ...
    setState(() {
      ...
      if (idx >= 0) {
        ...
        _panier.add(_LignePanier(
          ...
          imageUrl:     produit['image_url'] as String?, // ← récupéré depuis Supabase
        ));
      }
    });
  ...
  Future<void> _confirmerEtEnregistrer() async {
    if (_panier.isEmpty) {
      _toast('Ajoutez au moins un produit', erreur: true);
      return;
    }
    if (_modePaiement == 'À crédit' && _nomClientCtrl.text.trim().isEmpty) {
      _toast('Entrez le nom du client pour le crédit', erreur: true);
      return;
    }
    ...
    try {
      ...
      // 2️⃣ Insérer les lignes avec image_url ← CORRECTION PRINCIPALE
      for (final ligne in _panier) {
        await supabase.from('ventes_items').insert({
          ...
          'image_url':     ligne.imageUrl,  // ← sauvegarde l'image du produit
        });
  ...
  /// ✅ Image réelle du produit avec fallback catégorie
                ...
                child: GestureDetector(
                  ...
                  child: AnimatedContainer(
                    ...
                    child: m.logoAsset != null
                        ? ClipRRect(
                            borderRadius:
                                BorderRadius.circular(sel ? 10 : 11),
                            child: Image.asset(
                              m.logoAsset!,
                              width: double.infinity,
...
class _QtyControl extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        decoration: BoxDecoration(
            color: _DS.fog, borderRadius: BorderRadius.circular(10)),
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          GestureDetector(
            onTap: onMinus,
            child: Container(
              ...
              decoration: BoxDecoration(
                  color: _DS.coralLight,
                  borderRadius: BorderRadius.circular(8)),
              child: const Icon(Icons.remove, color: _DS.coral, size: 16),
            ),
          ),
          SizedBox(
            width: 34,
