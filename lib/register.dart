...
class _RegisterPageState extends State<RegisterPage>
  ...
  final _subs = [
    ...
    'Votre code secret de connexion'
  ];
  final _icons = [
  ...
  bool _etapeValide() {
    switch (_etape) {
      case 0:
        return _prenomCtrl.text.trim().isNotEmpty &&
            _nomCtrl.text.trim().isNotEmpty &&
            _valTel(_telephoneCtrl.text) == null;
      case 1:
        return _boutiqueCtrl.text.trim().isNotEmpty &&
            _villeCtrl.text.trim().isNotEmpty;
      case 2:
        return _valPin(_pinCtrl.text) == null &&
  ...
  try {
    ...
    // 1. Hash du PIN
    // 1. Hash du PIN (On utilise le numéro de téléphone comme "sel")
final String pinHashe = _db.hashPin(pinSaisi  );
  ...
  Widget _etape0() => Column(children: [
        Row(children: [
          Expanded(
              child: _champ(_prenomCtrl, 'Prénom', Icons.person_outline_rounded,
                  validator: (v) => _req(v, 'Prénom'))),
          const SizedBox(width: 12),
          Expanded(
              child: _champ(_nomCtrl, 'Nom', Icons.person_outline_rounded,
